...
Die Episode-Detailansicht wird durch dynamisch generierte Hintergrundbilder visuell aufgewertet. Statt eines statischen Banners wird aus dem Fanart einer Episode ein maßgeschneideres Hintergrundbild erzeugt, das proportional skaliert und zentriert auf einer Canvas dargestellt wird. Dies schafft ein konsistenteres visuelles Erlebnis und erhöht die Erkennbarkeit jeder Episode.

### Funktionsweise

Beim ersten Aufruf einer Episode-Detailseite lädt die Anwendung das Episode-Fanart und generiert daraus automatisch ein Hintergrundbild:

1. **Fanart-Laden:** Das Fanart-Bild der Episode wird aus der Datenbank geladen.
...
Wenn der Media-Scanner ein neues oder aktualisiertes Fanart für eine Episode findet:
- Das generierte Hintergrundbild wird als „überarbeitungsbedürftig" markiert
- Beim nächsten Aufruf der Episode wird ein neues Hintergrundbild aus dem aktualisierten Fanart erzeugt
- Das alte generierte Bild wird aus der Datenbank entfernt

### Einschränkungen

- **Fanart erforderlich:** Ist für eine Episode kein Fanart vorhanden, wird das Feature nicht aktiv und es wird stattdessen das existierende Banner/Fanart-Bild verwendet.
