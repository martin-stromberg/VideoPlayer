            ...
            try
            {
                ...
                try
                {
                    if (!currentSession.Job.Failed)
                    {
                        ...
                        var existingItem = GetExistingDownloadedItemAsync(mediaItem, currentSession.Job.CopyType).Wait<MediaItem>();
                        if (existingItem is null)
                        {
                            var collection = mediaLibrary.GetMediaItemCollectionAsync(mediaItem.ParentCollectionId)
                                                          .Wait<MediaItemCollection>();
                            var source = mediaLibrary.GetSourceAsync(collection.MediaSourceId).Wait<MediaSource>();
