...
namespace Mediathek.Services.MediaLibrary.Downloads
{
    ...
    public class DownloadSession: BaseModel
    {
        ...
        public Database.Models.DownloadJob Job
        {
            get
            {
                return GetProperty<Database.Models.DownloadJob>();
            }
            private set
            {
                SetProperty<Database.Models.DownloadJob>(value);
            }
        }
        ...
        public DownloadStatus Status
        {
            get
            {
                return GetProperty<DownloadStatus>();
            }
            private set
            {
                SetProperty<DownloadStatus>(value);
            }
        }
        ...
        {
            return new DownloadSession() { Job = null, Item = item, Status = DownloadStatus.Finished, Progress = 100 };
        }

        internal static DownloadSession CreateFromJob(Database.Models.DownloadJob job)
        {
            return new DownloadSession() { Job = job, Item = null, Status = DownloadStatus.Waiting, Progress = 0 };
        }
