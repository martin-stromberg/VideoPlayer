...
namespace VideoMeister.Services.Samba
{
    public class SambaShare
    {
        ...
        public void Connect()
        {
            ...
            {
                loginStatus = smbClient.Login("", username, password);
                if (loginStatus != NTStatus.STATUS_SUCCESS)
                    Disconnect();
            }
            CheckConnected();
        ...
        {
            return List(path)
                .Where(dir => ((uint)dir.FileAttributes & (uint)SMBLibrary.FileAttributes.Normal) == (uint)SMBLibrary.FileAttributes.Normal);
        }

        internal void DownloadFile(string sourcePath, string destPath)
        {
            CheckConnected();
