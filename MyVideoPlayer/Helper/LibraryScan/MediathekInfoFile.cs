...
namespace MyVideoPlayer.Helper.LibraryScan
{
    ...
    {
        public enum VideoType { Movie, TVShow }
        ...
        private enum CurrentInfo { None, Website }
        ...
        private string findTag(string source, string tagName, string proprtyName, string proprtyValue, string returnPropertyName)
        {
        ...
        private async Task LoadInfoFromZDFWebsiteAsync(string html)
        {
            ...
            using (HttpClient client = new HttpClient())
                try
                {
                    ...
                    var episodeInfo = findTag(body, "span", "class", "teaser-cat", string.Empty)
                        .Trim()
                        .Split(',');
