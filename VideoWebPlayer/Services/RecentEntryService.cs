...
public class RecentEntryService
{
    ...
    public async Task AddMovieCollectionAsync(MovieCollection collection)
    {
        ...
        // F�ge Collection hinzu, falls nicht vorhanden
    ...
    public async Task AddTVShowAsync(TVShow show)
    {
        ...
        // F�ge Show hinzu, falls nicht vorhanden
    ...
    {
        await ClearCorruptEntriesAsync();
        // Pr�fe, ob die Show schon gelistet ist
        ...
        // Finde alle Eintr�ge zu Staffeln/Episoden dieser Show
        ...
        foreach (var entry in entries)
        {
            if (entry.Type == RecentEntryType.TVShowSeason)
            {
                ...
                if (entrySeason != null)
                {
                    ...
                    {
                        // Sp�tere Staffel vorhanden, neue Staffel hinzuf�gen
                        shouldAdd = true;
                    }
                    else if (entrySeasonNumber < thisSeasonNumber)
                    {
                        // Fr�here Staffel vorhanden, neue Staffel NICHT hinzuf�gen
                        return;
                    }
                    else
                    {
                        // Gleiche Staffel, NICHT hinzuf�gen
                        return;
                    }
                }
            }
            else if (entry.Type == RecentEntryType.TVShowEpisode)
            {
                ...
                if (entryEpisode != null)
                {
                    ...
                    if (entrySeason != null)
                    {
                        ...
                        if (entrySeasonNumber > thisSeasonNumber)
                        {
                            // Episode einer sp�teren Staffel vorhanden, neue Staffel hinzuf�gen
                            shouldAdd = true;
                        }
                        else if (entrySeasonNumber < thisSeasonNumber)
                        {
                            // Episode einer fr�heren Staffel vorhanden, neue Staffel NICHT hinzuf�gen
                            return;
                        }
                        else
                        {
                            // Episode der gleichen Staffel vorhanden, neue Staffel hinzuf�gen
                            shouldAdd = true;
                        }
                    }
                }
        ...
        // Entferne alle Eintr�ge zu sp�teren Staffeln und deren Episoden
        if (shouldAdd)
        {
            ...
            // F�ge Staffel hinzu, falls nicht vorhanden
    ...
    public async Task AddTVShowEpisodeAsync(TVShowEpisode episode)
    {
        ...
        if (season is null)
            return;
        // Pr�fe, ob die Show oder Staffel schon gelistet ist
        ...
        // Pr�fe, ob ein Eintrag mit fr�herem Ver�ffentlichungsdatum existiert
